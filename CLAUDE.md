...
- `apps/splat-camera-export` — Vite/React/**PlayCanvas** app that loads a 3D Gaussian Splat capture of a real site, imports the camera layout from a `sample-app` `scene.json`, and exports one rendered image per camera. Implemented from `apps/splat-camera-export/specs/spec.md`. Depends on the SDK for **types only** (`Vec3`, `Quat`) — it renders, it does not analyse. See `apps/splat-camera-export/CLAUDE.md` for commands and its render-path traps.

Where `sample-app` answers *how much of the volume is covered*, `splat-camera-export`
answers *what each camera actually sees*. The `scene.json` camera format (sample-app spec
§14.3) is the contract between them.

`npm install` at the repo root wires all three up.
...
- App — `apps/splat-camera-export/ai/` — the same seven.
...
- **Read the relevant spec before changing anything.** `packages/camera-coverage-sdk/specs/spec.md`, `apps/sample-app/specs/spec.md`, and `apps/splat-camera-export/specs/spec.md` are the source of truth for intended behavior, not just background reading — check the applicable section(s) before touching engine or app code.
